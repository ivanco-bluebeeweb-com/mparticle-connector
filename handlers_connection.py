...
async def connect_mparticle_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    return ActionResult.ok(rec, summary=f"Connected mParticle ({rec['label']}).")
...
async def disconnect_mparticle_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    conns = await _load_conns(ctx)
    if not conns:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected mParticle connection.")
