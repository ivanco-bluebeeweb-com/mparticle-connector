"""Connection lifecycle for mParticle Connector."""
...
@chat.function(
    "connect_mparticle",
    "Connect mParticle account via credentials.",
    action_type="write",
    chain_callable=True,
    event="mparticle-connector.connect_mparticle",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_mparticle(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect mParticle Connector."""
    client = mParticleClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "mParticle Account",
...
@chat.function(
    "list_connections",
    "List connected mParticle accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_mparticle",
    "Disconnect mParticle account.",
    action_type="write",
    chain_callable=True,
    event="mparticle-connector.disconnect_mparticle",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_mparticle(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
